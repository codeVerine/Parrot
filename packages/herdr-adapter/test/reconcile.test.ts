...
test("reconciliation uses snapshot and agent list and emits a delta", async () => {
  const fake = new FakeHerdr(); const raw = await fake.startAgent({ name: "codex-reviewer", argv: ["codex"], workspace_id: "workspace-1" }, 100);
...
test("reconciliation binds a newly discovered pane without using pane_id as identity", async () => {
  const fake = new FakeHerdr(); await fake.startAgent({ name: "codex-reviewer", argv: ["codex"], workspace_id: "workspace-1" }, 100);
